...
class ForReviewSendToCoderPage(CodingCompletePage):
    """Workflow for sending an Outstanding episode to another coder for review."""

    MY_ACTIONS_BUTTON = 'button:has-text("My Actions"), button[aria-label="My Actions"]'
    CODER_SEARCH_INPUTS = [
        'input[placeholder="Search coders..."]',
        'input[placeholder*="Search coders"]',
        'input[aria-label*="Search coders"]',
        'role=textbox[name="Search coders..."]',
    ]
    ...
    def send_for_review_to_coder(
        self,
        coder_name: str = 'Prabhu',
        reason: str = 'Sai coder sends this episode to prabhu coder',
    ) -> bool:
        """Open My Actions, choose coder, add reason, and send for review."""
        if not self._open_my_actions_for_review():
            print('  ! My Actions review panel did not open')
            return False

        if not self._search_and_select_coder(coder_name):
            print(f'  ! Coder not selected: {coder_name}')
            return False
        ...
        if not self._wait_for_review_sent_status(coder_name):
        ...
        print(f'  [OK] Episode {episode_identifier} sent for review to coder: {coder_name}')
        return True
    ...
    def _select_for_review_action(self) -> bool:
        ...
        if self._for_review_submenu_is_visible() and self._visible_coder_search():
            return True
    ...
    def _click_review_submenu_item(self) -> bool:
        ...
        submenu_selectors = [
            ...
            'text=Send For Review',
            '.submenu-tabs button:has-text("Coders")',
            '[role="menuitem"]:has-text("Coders")',
        ]
    ...
    def _review_panel_is_visible(self) -> bool:
        ...
        selectors.extend(self.CODER_SEARCH_INPUTS)
    ...
    def _visible_coder_search(self) -> bool:
    ...
    def _visible_for_review_submenu(self):
        try:
            ...
            for index in range(submenus.count()):
                ...
                try:
                    search = submenu.locator('input.submenu-search-input[placeholder*="Search coders"]').first
    ...
    def _search_and_select_coder(self, coder_name: str) -> bool:
        ...
        if not search:
            try:
                search_inputs = self.page.locator('input.submenu-search-input[placeholder*="Search coders"]')
        ...
        if search:
            try:
                ...
                search.fill(coder_name.lower())
                self._safe_wait(700)
                print(f'  [OK] Coder searched: {coder_name.lower()}')
            except Exception:
                pass
            ...
            try:
                ...
                for index in range(rows.count()):
                    ...
                    try:
                        ...
                        continue
                    if coder_name.lower() not in row_text.lower():
                        continue
                    ...
                    if not self._radio_is_checked(submenu):
                        continue
                    print(f'  [OK] Coder selected: {coder_name}')
        ...
        if self._select_coder_radio_with_dom(coder_name, timeout=10000):
            self._safe_wait(300)
            print(f'  [OK] Coder selected: {coder_name}')
            self._wait_for_review_reason_input()
            return True
        ...
        coder_selectors = [
            f'label:has-text("{coder_name}") input[type="radio"]',
            f'.dropdown-item:has-text("{coder_name}") input[type="radio"]',
            f'[role="radio"]:has-text("{coder_name}")',
        ]

        for selector in coder_selectors:
            try:
    ...
    def _select_coder_radio_with_dom(self, coder_name: str, timeout: int = 10000) -> bool:
        start = time.time()
        while time.time() - start < timeout / 1000:
            try:
                selected = self.page.evaluate(
                    """coderName => {
                        const wanted = coderName.toLowerCase();
    ...
    def _wait_for_review_sent_status(self, coder_name: str, timeout: int = 60000) -> bool:
        start = time.time()
        success_selectors = [
...
if __name__ == '__main__':
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            review = ForReviewSendToCoderPage(page)
            ...
            def fail_step(msg: str, exc: Exception | None = None):
                ...
                screenshot_path = debug_dir / f'for_review_sendtocoder_failure_{ts}.png'
                html_path = debug_dir / f'for_review_sendtocoder_failure_{ts}.html'
            ...
            def run_for_review_scenario(name: str, open_coding_panel):
                ...
                print('Sending episode for review to coder...')
                try:
                    if not review.send_for_review_to_coder(
                        coder_name='Prabhu',
                        reason='Sai coder sends this episode to prabhu coder',
                    ):
                        raise AssertionError('send_for_review_to_coder returned False')
            ...
            print('\nFor Review Send to Coder run finished')
        except Exception:
            if exit_code[0] == 0:
                exit_code[0] = 1
        finally:
